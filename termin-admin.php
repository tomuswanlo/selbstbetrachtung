...
if ($isLoggedIn && $_SERVER['REQUEST_METHOD'] === 'POST' && checkCsrf()) {
    ...
    if ($do === 'add_blocked_date' || $do === 'update_blocked_group') {
        ...
        if ($do === 'update_blocked_group') {
            $ids = array_filter(array_map('intval', explode(',', (string) ($_POST['ids'] ?? ''))));
            $result = Booking::updateBlockedGroup($pdo, $ids, $dateFrom, $dateTo, $reason);
        } else {
    ...
    if ($do === 'delete_blocked_group') {
        $ids = array_filter(array_map('intval', explode(',', (string) ($_POST['ids'] ?? ''))));
        Booking::deleteBlockedGroup($pdo, $ids);
        redirectBack();
    }
    ...
    if ($do === 'add_manual_block' || $do === 'update_manual_block_group') {
        ...
        // Keine Uhrzeit angegeben => ganzer Tag blockiert.
        if ($start === '' || $end === '') {
...
$realBookings = array_values(array_filter($upcomingAll, static fn(array $u): bool => $u['type'] !== 'block'));
$blockGroups = Booking::groupBlockRows(array_values(array_filter($upcomingAll, static fn(array $u): bool => $u['type'] === 'block')));

// "Kommende Termine": echte Buchungen, blockierte Zeiträume und ganztägige Sperrtage
// (jeweils als Von–Bis-Gruppe) in einer gemeinsamen, chronologisch sortierten Übersicht.
$today = (new DateTimeImmutable('now', new DateTimeZone('Europe/Berlin')))->format('Y-m-d');
$agenda = [];
foreach ($realBookings as $u) {
    $agenda[] = ['kind' => 'booking', 'date' => $u['date'], 'sortKey' => $u['start_time'], 'data' => $u];
}
foreach ($blockGroups as $g) {
    $agenda[] = ['kind' => 'block_group', 'date' => $g['date_from'], 'sortKey' => $g['start_time'], 'data' => $g];
}
foreach ($blockedGroups as $g) {
    if ($g['date_to'] >= $today) {
        $agenda[] = ['kind' => 'blocked_group', 'date' => $g['date_from'], 'sortKey' => '00:00', 'data' => $g];
    }
}
...
<?php else: ?>
  ...
  <section class="card" id="sperrtage">
    ...
    <table>
      <thead><tr><th>Von</th><th>Bis</th><th>Grund</th><th></th></tr></thead>
      <tbody>
      <?php foreach ($blockedGroups as $g): ?>
        <tr>
          <td><?= htmlspecialchars((new DateTimeImmutable($g['date_from']))->format('d.m.Y')) ?></td>
          <td><?= htmlspecialchars((new DateTimeImmutable($g['date_to']))->format('d.m.Y')) ?></td>
          <td><?= htmlspecialchars((string) $g['reason']) ?></td>
          <td style="display:flex; gap:.4rem;">
            <a class="btn" style="text-decoration:none" href="?edit_blocked=<?= (int) $g['ids'][0] ?>#sperrtage">Bearbeiten</a>
            <form method="post" style="margin:0" onsubmit="return confirm('Diesen Zeitraum wirklich stornieren?');">
      ...
      <?php endforeach; ?>
      <?php if (!$blockedGroups): ?><tr><td colspan="4" class="muted">Keine Sperrtage eingetragen.</td></tr><?php endif; ?>
      </tbody>
    </table>
    <form class="inline" method="post">
      <input type="hidden" name="do" value="<?= $editBlockedGroup ? 'update_blocked_group' : 'add_blocked_date' ?>">
      <input type="hidden" name="csrf" value="<?= htmlspecialchars($csrf) ?>">
      <?php if ($editBlockedGroup): ?><input type="hidden" name="ids" value="<?= htmlspecialchars(implode(',', $editBlockedGroup['ids'])) ?>"><?php endif; ?>
      <label>Von <input type="date" name="date_from" value="<?= $editBlockedGroup ? htmlspecialchars($editBlockedGroup['date_from']) : '' ?>" required></label>
      <label>Bis (optional, für mehrere Tage) <input type="date" name="date_to" value="<?= $editBlockedGroup ? htmlspecialchars($editBlockedGroup['date_to']) : '' ?>"></label>
      <label>Grund <input type="text" name="reason" placeholder="z. B. Urlaub" value="<?= $editBlockedGroup ? htmlspecialchars((string) $editBlockedGroup['reason']) : '' ?>"></label>
      <button type="submit" class="btn"><?= $editBlockedGroup ? 'Speichern' : 'Sperren' ?></button>
      <?php if ($editBlockedGroup): ?><a href="termin-admin.php#sperrtage" style="color:var(--ink-mute)">Abbrechen</a><?php endif; ?>
    </form>
  </section>
